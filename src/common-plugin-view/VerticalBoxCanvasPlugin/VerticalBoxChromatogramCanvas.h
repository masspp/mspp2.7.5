 ...
 * @brief VerticalBoxChromatogramCanvas.cpp用ヘッダ
 * @author OKADA, H.
 * @date 2010.11
...
namespace kome {
	...
	namespace window {
		 ...
		 * @brief クロマトグラムをパネル以上に縦に並べて表示するクラス
		 */
		...
		public:
			 ...
			 * @brief gets base Chromatogram object. 個別のスペクトルを描画する際は、該当するspectrum objectを返す。それ以外の場合は全てのスペクトルを合成したobjectを返す。
			 ...
			 * @brief this method is called when prepareing (override method). 取得しているスペクトルはtransformed Spectrumで、ピーク情報が含まれないためピーク値が表示できない。本関数でtransformed SpectrumをオリジナルのSpectrumに変換する。
		...
		public:
			 ...
			 * @brief Stacked View Settings がクリックされたとき呼ばれる
