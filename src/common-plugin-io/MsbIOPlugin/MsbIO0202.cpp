...
void MsbIO0202::createStringTable( char** stringTable ) {
	// get size
	unsigned long stringTableSize = m_fileHeader[ 24 ].val.ul;
	...
	*stringTable = st;
	...
	// create buff
	unsigned int buffSize = m_fileHeader[ 23 ].val.ul;
	unsigned char* buff = new unsigned char[ buffSize ];

	// read string table
	fileseek( m_fp, m_fileHeader[ 22 ].val.ull, SEEK_SET );
...
void MsbIO0202::writeDataGroupNodes(
	...
	if( stReturnValue != size ){							// @date 2013/06/27 <Add> OKADA
		LOG_ERROR( FMT( "Writing MSB file error in writeDataGroupNodes(). Return value and item size of fwrite() disagree. Return value=%d, Item size=%d. errno=%d", stReturnValue, size, errno ) );	// @date 2013/06/27 <Add> OKADA
...
void MsbIO0202::onReadMsb( MsbSample& sample ) {
	...
	unsigned long long specGroupUserPropAlign = m_fileHeader[ 12 ].val.ull;
	unsigned long long chromAlign		 = m_fileHeader[ 14 ].val.ull;
	unsigned long long chromPropAlign	 = m_fileHeader[ 15 ].val.ull;
	unsigned long long chromUserPropAlign	 = m_fileHeader[ 16 ].val.ull;
	unsigned long long spectrumAlign	  = m_fileHeader[ 18 ].val.ull;
	unsigned long long specPartAlign	  = m_fileHeader[ 19 ].val.ull;
	unsigned long long specPropAlign	  = m_fileHeader[ 20 ].val.ull;
	unsigned long long specUserPropAlign	  = m_fileHeader[ 21 ].val.ull;
...
bool MsbIO0202::onWriteMsb(
	...
	m_fileHeader[ 12 ].val.ull = filetell( m_fp );
	...
	m_fileHeader[ 13 ].val.ull = filetell( m_fp );   // Chromatogram Data Align
	...
	m_fileHeader[ 14 ].val.ull = filetell( m_fp );					  // Chromatograms Align
	...
	m_fileHeader[ 15 ].val.ull = filetell( m_fp );					 // Chromatogram Properties Align
	...
	m_fileHeader[ 16 ].val.ull = filetell( m_fp );					 // Chromatogram User Properties Align
	...
	m_fileHeader[ 17 ].val.ull = filetell( m_fp );   // Spectrum Data Align
	...
	m_fileHeader[ 18 ].val.ull = filetell( m_fp );					  // Spectrum Align
